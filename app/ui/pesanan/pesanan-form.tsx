...
/**
 * Form pesanan bersama untuk mode "tambah" (create) dan "ubah" (edit).
 * Semua state, perhitungan biaya, dan JSX di sini dipakai kedua mode;
 * perbedaannya hanya: server action, prefill nilai awal, defaultValue, dan
 * label tombol submit. File create-form.tsx & edit-form.tsx tinggal membungkus
 * komponen ini dengan mode yang sesuai — menghilangkan duplikasi ±400 baris.
 */
...
import { createPesanan, updatePesanan, State } from "@/app/lib/actions";
...
export type PesananFormMode = "create" | "edit";
...
export default function PesananForm({
  ...
  mode: PesananFormMode;
  // Wajib diisi saat mode "edit" (sumber prefill & id untuk server action).
  pesanan?: TabelPesanan;
  // Item pesanan tersimpan untuk prefill saat mode "edit".
  ...
  const [state, formAction] = useActionState(
    isEdit && pesanan ? updatePesanan.bind(null, pesanan.id) : createPesanan,
    initialState,
  );

  // Mode edit: prefill item dari item_pesanan yang tersimpan; snapshot nama
  // dipetakan kembali ke id opsi (layanan/parfum). Diskon sudah menyimpan id
  // langsung. Mode create: mulai dari satu baris kosong.
  const [items, setItems] = useState<ItemRow[]>(() =>
    isEdit && initialItems && initialItems.length > 0
  ...
  const nextKey = useRef(
    isEdit && initialItems && initialItems.length > 0 ? initialItems.length : 1,
  );

  const [antarJemputYt, setAntarJemputYt] = useState(
    isEdit ? (pesanan?.antar_jemput_yt ?? "tidak") : "tidak",
  );
  // Mode edit: snapshot nama antar jemput dipetakan kembali ke id opsi saat ini
  const [antarJemputId, setAntarJemputId] = useState(() =>
    isEdit
      ? optionsAntarJemput.find(
          (option) =>
            option.nama_antar_jemput === pesanan?.nama_antar_jemput_snapshot,
        )?.id ?? ""
      : "",
  );
  const [jumlahBayar, setJumlahBayar] = useState(
    isEdit ? String(pesanan?.jumlah_bayar ?? 0) : "0",
  );
  ...
  const itemHitung = items.map((item) => hitungItem(item, layananMap, diskonMap));
    ...
    <form action={formAction}>
      ...
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-6">
          {/* Kolom kiri: Pelanggan, Item Layanan, Antar Jemput */}
          <div>
            {/* Pelanggan */}
            <div className="mb-4">
              ...
              <SelectPelanggan
                ...
                defaultValue={isEdit ? pesanan?.pelanggan_id : undefined}
              />
          ...
          {/* Kolom kanan: Ringkasan, Metode Pembayaran, Jumlah Bayar, Catatan */}
          <div>
            ...
            <div className="mb-4">
              ...
              <div>
                <SelectPopup
                  ...
                  defaultValue={isEdit ? (pesanan?.metode_pembayaran ?? "") : undefined}
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                <textarea
                  ...
                  name="catatan"
                  rows={3}
                  defaultValue={isEdit ? (pesanan?.catatan ?? "") : undefined}
      ...
      <div className="mt-6 flex justify-end gap-4">
        ...
        <Button type="submit">{isEdit ? "Update Pesanan" : "Tambah Pesanan"}</Button>
      </div>
