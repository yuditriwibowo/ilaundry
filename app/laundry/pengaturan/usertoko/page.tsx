...
import Table from "@/app/ui/usertoko/table";
import { CreateUserToko } from "@/app/ui/usertoko/buttons";
import { UserTokoTableSkeleton } from "@/app/ui/skeletons";
import { Suspense } from "react";
import { fetchUserTokoPages } from "@/app/lib/data";
      ...
      <div className="flex h-full w-full flex-col -mt-2">
        <div className="sticky top-0 z-10 bg-gradient-to-b from-primary-400 to-primary-800 pb-6 px-4 pt-6 -mx-4 rounded-b-xl md:static md:bg-none md:bg-gray-50 md:pb-0 md:px-0 md:pt-0 md:mx-0 md:rounded-b-none">
          <div className="flex w-full items-center justify-between">
              <h1 className={`text-2xl text-white md:text-gray-900`}>Pengaturan User Toko</h1>
           </div>
           <div className="mt-4 flex items-center justify-between gap-2 md:mt-6 short-screen:mt-2">
              <Search placeholder="Cari User Toko..." />
              <CreateUserToko />
           </div>
          </div>
          <div className="flex-1 overflow-y-auto scrollbar-hide min-h-0">
            <Suspense key={query + currentPage} fallback={<UserTokoTableSkeleton />}>
